...
pub fn sum_param(arg0: std::collections::BTreeMap<i64, i64>) -> i64 {
  {
    let mut acc = 0;
    for (k, v) in arg0.into_iter() {
      acc = acc + v;
    }
    acc
  }
...
fn main() {
  let arg0 = map_insert(map_insert(std::collections::BTreeMap::new(), 3, 4), 5, 6);
  let result = sum_param(arg0);
