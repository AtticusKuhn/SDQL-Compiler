...
- Terms (DeBruijn): typed surface terms `STermLoc2`/`SProg2` and typed core terms `TermLoc2`/`Prog2`, both indexed by an explicit context `ctx : List _` and using `Mem` proofs for variables. Builtins include `And`, `Or`, `Eq`, `Leq`, `Sub`, `StrEndsWith`, `Dom`, `Range`, `DateLit`, `Year`, and `Concat`.
