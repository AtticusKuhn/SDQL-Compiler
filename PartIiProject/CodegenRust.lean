...
mutual
  ...
  partial def compile {ty : _root_.Ty} : _root_.Term' (fun _ => String) ty → Rust.Expr
  | .var v => .var v
  | .constInt n => .litInt n
  | .constBool b => .litBool b
  | .constString s => .litString s
  | .constRecord fields => .tuple (compileRecordFields fields)
  | @_root_.Term'.emptyDict _ _ _ => .mapEmpty
  ...
  partial def compileRecordFields
    : {l : List _root_.Ty} → _root_.HList (_root_.Term' (fun _ => String)) l → List Rust.Expr
    | [], .nil => []
    | _ :: ts, .cons h t => compile h :: compileRecordFields t
end
