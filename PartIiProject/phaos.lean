...
inductive Term' (rep : Ty → Type) : Ty → Type
  ...
  | lam   : {dom ran : Ty} →(rep dom → Term' rep ran) → Term' rep (.fn dom ran)
  | app   : {dom ran : Ty} →Term' rep (.fn dom ran) → Term' rep dom → Term' rep ran
  | let   : {ty₁ ty₂ : Ty} →Term' rep ty₁ → (rep ty₁ → Term' rep ty₂) → Term' rep ty₂
...
def countVars {ty : Ty} : Term' (fun _ => Unit) ty → Nat
  | .var _    => 1
  | .const _  => 0
...
def prettyAux  {ty : Ty} (e : Term' (fun _ => String) ty) (i : Nat := 0) : String :=
  match e with
  | .var s     => s
  | .const n   => toString n
  | .app f a   => s!"({prettyAux f i} {prettyAux a i})"
  | .plus a b  => s!"({prettyAux a i} + {prettyAux b i})"
  | .lam f     =>
    let x := natToLetter i
    s!"(fun {x} => {prettyAux (f x) (i+1)})"
  | .let a b  =>
    let x := natToLetter i
    s!"(let {x} = {prettyAux a i} in {prettyAux (b x) (i+1)})"

def pretty {ty : Ty} (e : Term' (fun _ => String) ty) (i : Nat := 0) : String :=
  match e with
  | .var s     => s
  | .const n   => toString n
  | .app f a   => s!"{prettyAux f i} {prettyAux a i}"
  ...
  | .let a b  =>
    let x := natToLetter i
    s!"let {x} = {prettyAux a i} in {pretty (b x) (i+1)}"
...
-/
def squash {ty : Ty} {rep : Ty → Type} : Term' (Term' rep) ty → Term' rep ty
 | .var e    => e
 | .const n  => .const n
...
def subst {ty1 ty2 : Ty} (e : {rep : Ty → Type} → rep ty1 → Term' rep ty2) (e' : Term ty1) : Term ty2 :=
  squash (e e')
...
-/
@[simp] def constFold {ty : Ty} {rep : Ty → Type}: Term' rep ty → Term' rep ty
  | .var x    => .var x
  | .const n  => .const n
...
-/
theorem constFold_sound {ty : Ty} (e : Term' Ty.denote ty) : denote (constFold e) = denote e := by
...
-- @[simp] def letSubst {ty : Ty} {rep : Ty → Type}: Term' rep ty → Term' rep ty
--   | .var x    => .var x
--   | .const n  => .const n
--   | .app f a  => .app (letSubst f) (letSubst a)
--   | .lam f    => .lam fun x => letSubst (f x)
--   | .let a b  => subst sorry sorry
--   | .plus a b => .plus (letSubst a) (letSubst b)



@[simp] def letSubst {ty : Ty} {rep : Ty → Type} :
  Term' (Term' rep) ty → Term' rep ty
  | .var e    => e
  | .const n  => .const n
  | .plus a b => .plus (letSubst a) (letSubst b)
  | .app f a  => .app (letSubst f) (letSubst a)
  | .lam f    => .lam (fun x => letSubst (f (.var x)))
  | .let a b  => letSubst (b (letSubst a))


-- A convenient wrapper for closed terms
@[simp]
def inlineLets {ty : Ty} (e : Term ty) : Term ty :=
  fun {rep} => letSubst (e (rep := Term' rep))

def inlineLets2  {ty : Ty} {rep : Ty → Type} (t : Term' rep ty) : Term' rep ty
  := sorry


theorem inlineSound {ty : Ty} (e : Term ty) : denote (inlineLets e) = denote e := by
  simp
  -- induction e
  -- all_goals simp
  all_goals sorry
  ...
  -- sorry
-- A small sanity check (pretty-printing with Strings as variable names):

-- A tiny example: let a := 5 in a + a
def ex : Term Ty.nat :=
  .let (.const 5) (fun a => .plus (.var a) (.var a))

#eval pretty ex           -- "let a = 5 in (a + a)"
#eval pretty (inlineLets ex) -- "5 + 5"
#eval pretty (constFold (inlineLets ex)) -- "10"

-- def Term'.fold {ty : Ty} {rep : Ty → Type} {B : Type} : Term' rep ty → B
@[simp] def betaReduction {ty : Ty} {rep : Ty → Type} :
  Term' (Term' rep) ty → Term' rep ty
  | .var e    => e
  | .const n  => .const n
  | .plus a b => .plus (betaReduction a) (betaReduction b)
  | .app (.lam f) x  => betaReduction (f (betaReduction x))
  | .app f x  => .app (betaReduction f) (betaReduction x)
  | .lam f    => .lam (fun x => betaReduction (f (.var x)))
  | .let a b  => .let (betaReduction a) (fun x => betaReduction (b (.var x)))

def betaReduce {ty : Ty} (e : Term ty) : Term ty :=
  fun {rep} => betaReduction (e (rep := Term' rep))

#eval pretty three_the_hard_way           -- "((fun a => (fun b => (a + b))) 1) 2"
#eval pretty (betaReduce three_the_hard_way) -- "(fun a => (1 + a)) 2"
#eval pretty (betaReduce (betaReduce three_the_hard_way)) -- "1 + 2"
#eval pretty (constFold (betaReduce (betaReduce three_the_hard_way))) -- "3"
