  ...
  fun {ctx} {ty} t =>
    ...
    --       (.mk bodyLoc (.dictInsert k₂ v₂ (.mk emptyLoc .emptyDict))))) =>
        ...
        |  .var (.tail _ (@Mem.head _ _ _)) =>
              do
                let k₂NoY : TermLoc2 (_ :: _ :: ctx) _ ←
                  Term2.underBinderLoc (Term2.underBinder Term2.dropUnusedHead) k₂
                let v₂NoY : TermLoc2 (_ :: _ :: ctx) _ ←
                  Term2.underBinderLoc (Term2.underBinder Term2.dropUnusedHead) v₂

                let σ : Term2.Subst (_ :: _ :: ctx) (_ :: _ :: ctx) :=
                  fun m =>
                    match m with
                    | .head _ => k₁.term
                    | .tail _ m =>
                ...
                let fusedBody : TermLoc2 (_ :: _ :: ctx) (.dict _ _) :=
                  .mk bodyLoc (.dictInsert (Term2.substLoc2 σ k₂NoY) (Term2.substLoc2 σ v₂NoY) emptyFused)
    ...
    | Term2.letin
        ...
        | .var (.head _), .var (.head _) =>
              do
                ...
                let k₂' := Term2.substLoc2 σ k₂NoY
                let v₂' := Term2.substLoc2 σ v₂NoY
                let emptyFused : TermLoc2 (_ :: _ :: ctx) (.dict _ _) := .mk emptyLoc .emptyDict
                let fusedBody : TermLoc2 (_ :: _ :: ctx) (.dict _ _) :=
